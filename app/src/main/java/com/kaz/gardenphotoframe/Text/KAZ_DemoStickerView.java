...
public abstract class KAZ_DemoStickerView extends FrameLayout implements View.OnClickListener {
    ...
    public KAZ_DemoStickerView(Context context, OnTouchSticker onTouchSticker) {
        super(context);
        init(context);
        this.onTouchSticker = onTouchSticker;
    }

    public KAZ_DemoStickerView(Context context, AttributeSet attrs, OnTouchSticker onTouchSticker) {
        super(context, attrs);
        init(context);
        this.onTouchSticker = onTouchSticker;
    }

    public KAZ_DemoStickerView(Context context, AttributeSet attrs, int defStyle, OnTouchSticker onTouchSticker) {
    ...
    private void init(Context context) {
        ...
        this.iv_delete.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                if (KAZ_DemoStickerView.this.getParent() != null) {
                    ViewGroup myCanvas = ((ViewGroup) KAZ_DemoStickerView.this.getParent());
                    myCanvas.removeView(KAZ_DemoStickerView.this);
                }
            }
        });
            ...
            if (view.getTag().equals("DraggableViewGroup")) {
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        KAZ_DemoStickerView.this.setX(KAZ_DemoStickerView.this.getX() + offsetX);
                        KAZ_DemoStickerView.this.setY(KAZ_DemoStickerView.this.getY() + offsetY);
                        move_orgX = event.getRawX();
                        move_orgY = event.getRawY();
                        break;
                ...
                switch (event.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        ...
                        this_orgX = KAZ_DemoStickerView.this.getX();
                        this_orgY = KAZ_DemoStickerView.this.getY();

                        scale_orgX = event.getRawX();
                        scale_orgY = event.getRawY();
                        scale_orgWidth = KAZ_DemoStickerView.this.getLayoutParams().width;
                        scale_orgHeight = KAZ_DemoStickerView.this.getLayoutParams().height;

                        rotate_orgX = event.getRawX();
                        rotate_orgY = event.getRawY();

                        centerX = KAZ_DemoStickerView.this.getX() +
                                ((View) KAZ_DemoStickerView.this.getParent()).getX() +
                                (float) KAZ_DemoStickerView.this.getWidth() / 2;
                        ...
                        centerY = KAZ_DemoStickerView.this.getY() +
                                ((View) KAZ_DemoStickerView.this.getParent()).getY() +
                                statusBarHeight +
                                (float) KAZ_DemoStickerView.this.getHeight() / 2;
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        if (length2 > length1
                            ...
                            KAZ_DemoStickerView.this.getLayoutParams().width += offset;
                            KAZ_DemoStickerView.this.getLayoutParams().height += offset;
                            onScaling(true);
                            //DraggableViewGroup.this.setX((float) (getX() - offset / 2));
                            //DraggableViewGroup.this.setY((float) (getY() - offset / 2));
                        } else if (length2 < length1
                                && (angle_diff < 25 || Math.abs(angle_diff - 180) < 25)
                                && KAZ_DemoStickerView.this.getLayoutParams().width > size / 2
                                && KAZ_DemoStickerView.this.getLayoutParams().height > size / 2) {
                            //scale down
                            double offsetX = Math.abs(event.getRawX() - scale_orgX);
                            double offsetY = Math.abs(event.getRawY() - scale_orgY);
                            double offset = Math.max(offsetX, offsetY);
    ...
    private class BorderView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            borderPaint.setColor(getResources().getColor(R.color.primary_second));
